...
void LagrangeElem2DNavierStokesTria3Node::prepareElemData(vector<vector<double> >& node_coords)
{
    ...
    for(gp=0; gp<nGP; gp++)
    {
      ...
      computeBasisFunctions2D(false, ELEM_SHAPE_TRIA, degree, param, xNode, yNode, &Nv[gp][0], &dNvdx[gp][0], &dNvdy[gp][0], Jac);
...
/*
int LagrangeElem2DNavierStokesTria3Node::calcStiffnessAndResidual(vector<vector<double> >& node_coords, double* elemData, double* timeData, VectorXd& solnPrev, VectorXd& solnPrev2, VectorXd& solnCur, VectorXd& solnDotCur, MatrixXd& Klocal, VectorXd& Flocal, double timeCur)
{
...
int LagrangeElem2DNavierStokesTria3Node::calcStiffnessAndResidual(vector<vector<double> >& node_coords, double* elemData, MatrixXd& Klocal, VectorXd& Flocal, double timeCur)
{
    ...
    double  xNode[npElem], yNode[npElem];
    for(ii=0;ii<npElem;ii++)
    {
      xNode[ii] = node_coords[nodeNums[ii]][0];
      yNode[ii] = node_coords[nodeNums[ii]][1];
...
int  LagrangeElem2DNavierStokesTria3Node::calcError(int index)
{
/*
    //cout << " Error ... " << endl;
